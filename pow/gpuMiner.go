...
func (g *GpuMiner)CheckRange(base []byte, difficulty *big.Int,  start uint64, n uint64) (string, error) {
	mulDivisorBytes := createDivisorByteArray(difficulty)

	_, err := g.queue.EnqueueWriteBuffer(g.prefix, true, 0, len(base), unsafe.Pointer(&base[0]), nil)
	if err != nil {
