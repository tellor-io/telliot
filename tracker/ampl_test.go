...
func mockAPI(req *http.Request) *http.Response {
	...
	switch {
	// mpleforth/BTC
	case contains(req, "https:// pi.kucoin.com/api/v1/market/stats?symbol=AMPL-BTC"):
	...
	case contains(req, "https:// pi-pub.bitfinex.com/v2/tickers?symbols=tAMPBTC"):
	...
	// Ampleforth/USD
	case contains(req, "https:// pi.coingecko.com/api/v3/simple/price?ids=ampleforth&vs_currencies=usd"):
		reqBody = `{"ampleforth":{"usd":2.15}}`
	case contains(req, "https:// pi.kucoin.com/api/v1/market/stats?symbol=AMPL-USDT"):
	...
	case contains(req, "https:// pi-pub.bitfinex.com/v2/tickers?symbols=tAMPUSDT"):
		reqBody = `[["tAMPUST",2.1658,30036.857127949996,2.1697,37123.11378477,0.5024,0.3031,2.16,106302.90115003,2.2746,1.5466]]`
	case contains(req, "https:// pi-pub.bitfinex.com/v2/tickers?symbols=tAMPUSD"):
		reqBody = `[["tAMPUSD",1.9711,30215.46857231,2.0035,34257.8592384,0.1128,0.0606,1.9729,558433.75188253,2.255,1.5608]]`
	case contains(req, "https:// in-api.cryptocompare.com/data/price?fsym=AMPL&tsyms=USD"):
		reqBody = `{"USD":2.193}`
	// Bitcoin
	case contains(req, "https:// pi.pro.coinbase.com/products/BTC-USD/ticker"):
	...
	case contains(req, "https:// pi.binance.com/api/v1/klines?symbol=BTCUSDT&interval=1d&limit=1"):
	...
	case contains(req, "https:// pi.coindesk.com/v1/bpi/currentprice.json"):
	...
	case contains(req, "https:// ww.bitstamp.net/api/v2/ticker/btcusd"):
	...
	case contains(req, "https:// pi.coinpaprika.com/v1/tickers/btc-bitcoin"):
	...
	case contains(req, "https:// pi.cryptowat.ch/markets/coinbase-pro/btcusd/price"):
		reqBody = `{"result":{"price":11693.93},"allowance":{"cost":4063246,"remaining":3950410844,"upgrade":"For unlimited API access, create an account at https:// ryptowat.ch"}}`
	case contains(req, "https:// pi.kraken.com/0/public/Ticker?pair=XBTUSD"):
	...
	case contains(req, "https:// pi-pub.bitfinex.com/v2/tickers?symbols=tBTCUSD"):
