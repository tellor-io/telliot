...
func TestCreateTracker(t *testing.T) {
	...
	balanceTracker, _ := createTracker("balance", logger, cfg, DB, client, nil, nil)
	...
	currentVariablesTracker, _ := createTracker("currentVariables", logger, cfg, DB, client, nil, nil)
	...
	disputeStatusTracker, _ := createTracker("disputeStatus", logger, cfg, DB, client, nil, nil)
	...
	gasTracker, _ := createTracker("gas", logger, cfg, DB, client, nil, nil)
	if gasTracker[0].String() != "GasTracker" {
		testutil.Ok(t, errors.Errorf("Expected GasTracker but got %s", gasTracker[0].String()))
	}

	tributeBalanceTracker, _ := createTracker("tributeBalance", logger, cfg, DB, client, nil, nil)
	...
	indexersTracker, err := createTracker("indexers", logger, cfg, DB, client, nil, nil)
	...
	disputeChecker, _ := createTracker("disputeChecker", logger, cfg, DB, client, nil, nil)
	...
	_, err = createTracker("badTracker", logger, cfg, DB, client, nil, nil)
